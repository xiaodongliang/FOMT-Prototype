...
$(document).ready (function () {
    ...
    if(customerNum > 0){
        ...
        $("#useremailimg").attr('src','images/customer.png');
         $('#username').val('Customer No.： ' + customerNum);
    }
    ...
    socket.on('au_one_more_model', function(msg){
        ...
        var html = '<div class="col-xs-12 col-sm-6 col-lg-8">'
            ...
            + '<br><h3><span class="label label-default">Part Number： ' + testJson.part_number + '</span></h3>   '
            + '<h3><span class="label label-default">Quantity： ' + testJson.part_count + '</span></h3>   '
            + '<h3><span class="label label-default">Material： ' + testJson.material_type + '</span></h3>   '
             +'<button class="btn btn-large btn-primary" id="btnRFQ-' +filenamewithoutext + '" >Review</button>'
            + '</div>';
                    ...
                    if (responsObj.translatingprogress) {
                        if (responsObj.translatingprogress == 'complete' && responsObj.urn != '') {
                           var stgJson =  JSON.parse(localStorage["RFQBasicInfo" + filenamewithoutext]);
                            ...
                            alert('Translation in the process. Try a moment later！');
                        }
                    }
                }
